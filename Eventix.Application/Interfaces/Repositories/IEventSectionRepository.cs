...
public interface IEventSectionRepository
{
    ...
    Task UpdateAsync(EventSection entity, CancellationToken cancellationToken = default);
    Task DeleteAsync(EventSection entity, CancellationToken cancellationToken = default);
    Task<bool> ExistsByEventAndVenueSectionAsync(Guid eventId, Guid venueSectionId, CancellationToken cancellationToken = default);
}